...
- 代码中涉及输出的对象必须经过安全筛选。
- 网络、链、能力白名单等基础参数必须从 config 派生，禁止在业务模块重复硬编码。
- 若发现同一配置在两处维护，优先重构为“单一配置源 + 派生使用”。
...
- 检查 provider/task/search 中是否存在硬编码网络数组（如 `networks: ["eth", "bsc"]`）。
- 若存在，需说明为何不能从 `apps/*/config` 派生，或完成重构后再提交。
