...
export async function searchAddressAssetsTaskWithEngine(input = {}, engine, options = {}) {
  ...
  const valuation = await enrichAssetsWithValuation(assets, base.network, {
    priceBatchQuery: typeof options?.priceBatchQuery === "function"
      ? options.priceBatchQuery
      : (items) => searchTokenPriceBatchTaskWithEngine({ items }, null),
  });
...
export async function searchPortfolioTaskWithEngine(input = {}, engine, options = {}) {
  ...
  if (withPrice) {
    ...
    const priced = await enrichAssetsWithValuation(mergedAssets, null, {
      priceBatchQuery: typeof options?.priceBatchQuery === "function"
        ? options.priceBatchQuery
        : (items) => searchTokenPriceBatchTaskWithEngine({ items }, null),
    });
