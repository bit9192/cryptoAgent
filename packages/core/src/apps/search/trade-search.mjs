...
function isTradeProvider(provider) {
  if (!provider || typeof provider !== "object") return false;
  const capabilities = Array.isArray(provider.capabilities) ? provider.capabilities : [];
  return capabilities.includes("trade") && typeof provider.searchTrade === "function";
}
...
export async function searchTradeWithProviders(input = {}, options = {}) {
  ...
  for (const provider of providers) {
    try {
      const rows = await provider.searchTrade({
        query,
        network: input?.network,
        limit,
      }, {
